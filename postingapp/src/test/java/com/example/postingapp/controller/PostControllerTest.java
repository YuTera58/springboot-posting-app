...
　　　よって、認可によってログインページにリダイレクトしたことを検証する場合、redirectedUrl()メソッドの引数も絶対パスで指定する必要があります。
    ...
    **/
    ...
    /* データが削除されていることをisEmpty()メソッドで検証する
     isEmpty()メソッドを使うことで、Optional<T>型（Tは任意の型）のオブジェクトが空かどうかを検証できます。
     2つ目と3つ目のテストメソッドでは、逆に投稿が削除されていないことを検証したいので、
     以下のようにisPresent()メソッドを使っています。
    **/
    @Test
    @WithUserDetails("taro.samurai@example.com")
    @Transactional
